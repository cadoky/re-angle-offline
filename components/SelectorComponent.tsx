...
const SelectorComponent: React.FC<SelectorProps> = ({ label, icon, value, options: menuOptions, onChange, translations: menuTranslations, helpTexts, optionImages, disabled = false, colorPicker = null }) => {
        ...
        <div className="dropdown-container w-full relative" ref={selRef} style={{ zIndex: isOpen ? 50 : undefined }}>
            ...
            {isOpen && !disabled && (
                <div className={`dropdown-menu rounded-2xl overflow-hidden animate-in fade-in duration-300 shadow-[0_20px_50px_rgba(0,0,0,1)] w-full custom-scrollbar ${openUpwards ? 'bottom-full mb-1' : 'top-full mt-1'}`}>
                    {menuOptions?.map((opt) => (
                        <button key={opt} onClick={(e) => { e.stopPropagation(); onChange(opt); setOpen(false); }} className="dropdown-item w-full text-left font-bold flex items-center gap-3">
                            {optionImages && optionImages[opt] && (
                                <img src={optionImages[opt]} alt={opt} className="w-8 h-8 rounded border border-white/10 object-cover" />
                            )}
                            <span>{menuTranslations ? menuTranslations[opt] : opt}</span>
                        </button>
                    ))}
                </div>
            )}
